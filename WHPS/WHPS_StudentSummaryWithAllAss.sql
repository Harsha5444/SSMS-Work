...
INNER JOIN main.k12school sch
	ON s.schoolid = sch.SchoolIdentifier
		AND s.Schoolyear = sch.Schoolyear
		AND s.Tenantid = sch.Tenantid
INNER JOIN [dbo].[WHPS_CC_export] cc
	ON cc.studentid = s.STUDENTID
		AND cast(cc.dateenrolled AS DATE) BETWEEN cast(s.entrydate AS DATE)
			AND cast(s.exitdate AS DATE)
INNER JOIN [dbo].[WHPS_Sections_export] x
	ON cc.sectionid = x.[ID]
INNER JOIN [dbo].[WHPS_Teachers_export] t
	ON x.teacher = t.id
INNER JOIN [dbo].[WHPS_Courses_export] C
	ON x.course_number = C.course_number
INNER JOIN [dbo].[WHPS_Terms_export] tm
	ON tm.id = x.[TermID]
		AND tm.schoolid = x.[SchoolID]
LEFT JOIN [dbo].[WHPS_Gen_export] g
	ON g.[gen name] = C.sched_department
...
LEFT JOIN [dbo].[WHPS_U_STUDENTSUSERFIELDS_export] uf
	ON uf.Student_Number = s.STUDENT_NUMBER
		AND uf.StudentsDCID = s.Studentid
LEFT JOIN [dbo].[WHPS_S_CT_STU_X_export] op
	ON op.Student_Number = s.STUDENT_NUMBER
		...
		FROM (
			...
			FROM (
				...
				INNER JOIN (
					...
					ON ss.STATE_STUDENT_ID = se.STATE_STUDENTNUMBER
				...
				INNER JOIN [Main].[WHPS_Students] se
					ON ss.SECONDARY_ID = se.Student_Number
						AND ss.schoolyear = se.schoolyear
				GROUP BY ss.schoolyear
					,se.SCHOOLID
					,ss.SECONDARY_ID
			...
			INNER JOIN (
				...
				ON (CASE WHEN l.AssessmentCode IN ('PSAT89', 'PSATNM') THEN 'PSAT' ELSE l.AssessmentCode END) = b.AssessmentCode
			...
			INNER JOIN (
				...
				ON (CASE WHEN l.AssessmentCode IN ('PSAT89', 'PSATNM') THEN 'PSAT' ELSE l.AssessmentCode END) = d.AssessmentCode
...
--update ss SET ss.AIMSWebPlus_Mathematics_ProfLevel=NULL,ss.AIMSWebPlus_Mathematics_ScaleScore=NULL
--,ss.AIMSWebPlus_OralReadingFluency_ProfLevel=NULL,ss.AIMSWebPlus_OralReadingFluency_ScaleScore=NULL
--,ss.AIMSWebPlus_Reading_ProfLevel=NULL,ss.AIMSWebPlus_Reading_ScaleScore=NULL
--,ss.[i-Ready_Mathematics_ProfLevel]=NULL,ss.[i-Ready_Mathematics_ScaleScore]=NULL
--,ss.[i-Ready_Reading_ProfLevel]=NULL,ss.[i-Ready_Reading_ScaleScore]=NULL
--,ss.NGSS_Science_ProfLevel=NULL,ss.NGSS_Science_ScaleScore=NULL,ss.SBAC_EnglishLanguageArts_ProfLevel=NULL
--,ss.SBAC_EnglishLanguageArts_ScaleScore=NULL,ss.SBAC_Mathematics_ProfLevel=NULL
--,ss.SBAC_Mathematics_ScaleScore=NULL,ss.STAR_Reading_ProfLevel=NULL,ss.STAR_Reading_ScaleScore=NULL
...
INNER JOIN (
	SELECT TenantId
		...
		,districtstudentid
		,schoolidentifier
		,[AIMSWebPlus_Mathematics_ProfLevel] [AIMSWebPlus_Mathematics_ProfLevel]
		,[AIMSWebPlus_Mathematics_ScaleScore] [AIMSWebPlus_Mathematics_ScaleScore]
		,[AIMSWebPlus_Oral Reading Fluency_ProfLevel] [AIMSWebPlus_OralReadingFluency_ProfLevel]
		,[AIMSWebPlus_Oral Reading Fluency_ScaleScore] [AIMSWebPlus_OralReadingFluency_ScaleScore]
		,[AIMSWebPlus_Reading_ProfLevel] [AIMSWebPlus_Reading_ProfLevel]
		,[AIMSWebPlus_Reading_ScaleScore] [AIMSWebPlus_Reading_ScaleScore]
		,[i-Ready_Mathematics_ProfLevel] [i-Ready_Mathematics_ProfLevel]
		,[i-Ready_Mathematics_ScaleScore] [i-Ready_Mathematics_ScaleScore]
		,[i-Ready_Reading_ProfLevel] [i-Ready_Reading_ProfLevel]
		,[i-Ready_Reading_ScaleScore] [i-Ready_Reading_ScaleScore]
		,[STAR_Reading_ProfLevel] [STAR_Reading_ProfLevel]
		,[STAR_Reading_ScaleScore] [STAR_Reading_ScaleScore]
	FROM (
	...
	ON ss.Student_Number = r.DistrictStudentId
		AND ss.schoolyear = r.schoolyear
		--update ss SET ss.NGSS_Science_ProfLevel=r.NGSS_Science_ProfLevel
		...
		--FROM WHPS_StudentSummaryWithAllAss ss
		--JOIN(
		--SELECT TenantId,schoolyear,districtstudentid,schoolidentifier
		--,[NGSS_Science_ProfLevel] [NGSS_Science_ProfLevel]
		--,[NGSS_Science_ScaleScore] [NGSS_Science_ScaleScore]
		--,[SBAC_English Language Arts_ProfLevel] [SBAC_EnglishLanguageArts_ProfLevel]
		--,[SBAC_English Language Arts_ScaleScore] [SBAC_EnglishLanguageArts_ScaleScore]
		--,[SBAC_Mathematics_ProfLevel] [SBAC_Mathematics_ProfLevel]
		--,[SBAC_Mathematics_ScaleScore] [SBAC_Mathematics_ScaleScore]
		-- FROM (
		--  SELECT distinct TenantId
		--  ,schoolyear
		--   --,schoolyear+1 schoolyear
		--   ,districtstudentid,schoolidentifier
		--   ,Assessment,SubjectAreaName
		--   ,Assessment + '_' + SubjectAreaName + '_ProfLevel' AS ColumnName
		--   ,ProficiencyDescription AS SValue   
		--  FROM WHPSAssessmentAllDS_Vw
		--  WHERE IsLatest = 1 and ProficiencyDescription is not null 
		...
		--   ,CAST(ScaleScore AS VARCHAR) AS SValue
		--  FROM WHPSAssessmentAllDS_Vw
		--  WHERE IsLatest = 1 and ScaleScore is not null 
		--  AND AssessmentCode in('SBAC','NGSS')
		--   --AND SchoolYear=2024
		--  AND SchoolYear in(2024,2023)
		--  ) AS source
		-- PIVOT(MAX(SValue) FOR ColumnName IN (
		...
		--,[SBAC_Mathematics_ScaleScore] )) AS p)r ON ss.Student_Number=r.DistrictStudentId
		--and ss.schoolyear=r.schoolyear
		--Teacher name update
		--====================
		--update ss set ss.Teacher=se.firstname+' '+se.LastOrSurname
		----select distinct ss.districtstaffid,Teacher,se.districtstaffid,se.firstname,se.LastOrSurname
		----,se.firstname+' '+se.LastOrSurname
		--from WHPS_StudentSummaryWithAllAss ss
		--join Main.K12StaffDemographics se on ss.districtstaffid=se.districtstaffid
