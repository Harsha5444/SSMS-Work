...
UPDATE ss
SET ss.ETHNICITY = isnull(se.race,'Not Selected')
FROM WHPS_StudentSummaryWithAllAss ss
JOIN aggrptk12studentdetails se ON ss.Student_Number = se.DistrictStudentId
	AND ss.SchoolIdentifier = se.SchoolIdentifier
	AND ss.SchoolYear = se.SchoolYear
	AND ss.TenantID = se.TenantId
WHERE ss.TenantID = 38